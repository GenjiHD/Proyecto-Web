...
// Watch cantidad y precio para actualizar subtotal e iva
...
onMounted(async () => {
    await getAllArticulos(); // Obtenemos todos los articulos
});

// Watch actualizar el id_articulo en compra cuando cambia la descripción seleccionada
...
const showToast = () => {
    if (mensaje.value) {
        if (mensaje.value[0] === 'Compra agregada con éxito') {
            ...
            errorToast('No se ha podido agregar la compra');
        }
    }
};
